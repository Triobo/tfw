...
HTMLElement.prototype.amILast = function () {
    var x = 0;
    if (this.myOrder == (this.parentNode.childNodes.length - 1))
        x = 1;
    return x;
}
...
var desktop = {
    ...
    init : function (id) {
        ...
        window.addEventListener("resize", function () {
            ...
            for (i = 0; i < desktop.resizingFunctions.length; i++)
                desktop.resizingFunctions[i]();
        }, false);
        var myURLparts = document.URL.split("?");
        desktop.URLbase = myURLparts[0];
        if (myURLparts.length > 1)
            desktop.URLparams = myURLparts[1].substr(2).split("/");
    },
    clean : function () {
        desktop.layers = [];
        ...
        isWorking : 0;
        if (desktop.mainLoaderTimer)
            clearTimeout(desktop.mainLoaderTimer);
    },
    ...
    working : function (now) {
        if (!desktop.isWorking) {
            ...
            if (now)
                ...
                desktop.mainLoaderTimer = setTimeout("desktop.hide();", 500);
    ...
    dialogMoveStart : function (e) {
        ...
        while (desktop.dialogMoveData.who.className != "tfwDialogContainer")
            desktop.dialogMoveData.who = desktop.dialogMoveData.who.parentNode;
        e.stopPropagation();
        e.preventDefault();
    },
...
var tfw = {
	...
	icon : function (params) {
		...
		var b = document.createElement("div");
		if (params.index)
			b.style.backgroundPositionX = (-params.index) + "px";
		element.add(b);
		element.disabled = 0;
		...
		if (params.disabled)
			element.disabled = params.disabled;
	...
	slider : function (params) {
		...
		if (params.valueStyle)
			v.style.cssText = params.valueStyle;
		v.style.textAlign = "right";
		v.value = sliderValue;
		v.onchange = function () {
			if (!this.value.match(/^\d+$/))
				this.value = 0;
			if (this.value < element.min)
				this.value = element.min;
			if (this.value > element.max)
				this.value = element.max;
		...
		if (params.postText) {
			...
			if (params.postStyle)
				p.style.cssText = params.postStyle;
		}
	...
	filebox : function (n) {
		...
		if (n.id)
			...
			x.id = "filebox";
		if (n.className)
			n.className = "tfwFilebox " + n.className;
		else
			n.className = "tfwFilebox";
		...
		if (n.path)
			...
			x.path = "";
		if (n.imgStyle)
			x.imgStyle = "style='" + n.imgStyle + "' ";
		else
			x.imgStyle = "";
		if (n.onloaded)
			x.onloaded = n.onloaded;
		else
			x.onloaded = null;
		if (n.onstart)
			x.onstart = n.onstart;
			...
			x.onstart = null;
		if (n.limitExtensions)
			x.limitExtensions = n.limitExtensions;
		else
			x.limitExtensions = "";
		x.uploading = 0;
		x.add(b = tfw.div({}));
		...
		x.upload = function (u) {
			...
			if (canbe) {
				...
				var lastl = lims.pop();
				if (lims.length)
					lim = "<b>" + lims.join("</b>, <b>") + "</b>" + tPRELOZ(" or ") + "<b>" + lastl + "</b>";
				else
					lim = "<b>" + lastl + "</b>";
	...
	dialog : function (co) {
		...
		var obal = tfw.div({
				...
				style : "left:" + Math.round((desktop.width - sirka) / 2) + "px;top:" + Math.round((desktop.height - vyska) / 2) + "px;width:" + sirka + "px;height:" + vyska + "px;"
			});
		...
		if (co.obsah)
			dlg.innerHTML = co.obsah;
		if (co.children)
			for (var i = 0; i < co.children.length; i++)
				dlg.add(c = co.children[i]);
		vnit.add(dlg.buttons = tfw.div({
					className : "buttonsBar"
				}));
		if (co.buttons)
			for (var i = 0; i < co.buttons.length; i++)
		...
		x.vyber = function (ord) {
			...
			if (novy != stary) {
				if ($(this.id + "-ousko-" + stary)) {
					...
					if ($(this.id + "-obsah-" + stary).onHide)
						$(this.id + "-obsah-" + stary).onHide();
				}
				$(this.id + "-obsah-" + novy).className = "zalozkySvisleObsah";
				$(this.id + "-ousko-" + novy).className += " aktivni";
				if ($(this.id + "-obsah-" + novy).onShow)
					$(this.id + "-obsah-" + novy).onShow();
				this.value = novy;
				this.vybrany = ord;
				if (this.onchange)
					this.onchange();
...
var prvek = {
	...
	seznamZatrzitek : function (co) {
		...
		x.className = c;
		if (co.style)
			x.style.cssText = co.style;
		if (co.onchange)
			x.onchange = co.onchange;
		if (co.seznam)
			for (var i = 0; i < co.seznam.length; i++)
				x.add(tfw.checkbox({
						...
						onchange : function () {
							...
							for (var i = 0; i < s.childNodes.length; i++)
								if (s.childNodes[i].value)
									y.push(s.childNodes[i].id.split("-")[1]);
							s._value = y.join(",");
							if (x.onchange)
								x.onchange();
						}
	...
	sloupec : function (co) {
		...
		if (co.obsah)
			co.innerHTML = co.obsah;
		if (co.width)
			co.style = "width:" + co.width + "px;" + ((co.style) ? co.style : "");
		return tfw.td(co);
	},
	/**
	...
	soubory : function (co) {
		...
		c.add(tfw.icon({
				...
				action : function (e) {
					...
					for (i = 0; i < c.length; i++)
						if (window.getComputedStyle(c[i]).display != "none")
							c[i].addClass("oznacen");
				}
			}));
		if (co.linky)
			c.add(tfw.icon({
					...
					action : function (e) {
						...
						var c = x.files;
						for (i = 0; i < c.length; i++)
							if ((window.getComputedStyle(c[i]).display != "none") && (!c[i].linky))
								c[i].addClass("oznacen");
					}
				}));
		...
		if (co.akceDvojklik)
			x.akceDvojklik = co.akceDvojklik;
		x.oznac = co.oznac;
		x._aktivni = 0;
		...
		x.appendFile = function (file) {
			...
			s.addEventListener("mousedown", function (e) {
				if ((e.ctrlKey) || (e.metaKey)) {
					if (this.hasClass("oznacen"))
						this.removeClass("oznacen");
					else {
			...
			s.addEventListener("dblclick", function (e) {
				for (var i = 0; i < this.parentNode.childNodes.length; i++)
					this.parentNode.childNodes[i].removeClass("oznacen");
				this.addClass("oznacen");
				e.preventDefault();
				e.stopPropagation();
				if (x.akceDvojklik) {
		...
		x.napoveda = function () {
			if (x.akcePridat)
				x.textNapovedy.style.display = x.files.length ? "none" : "block";
		}
	...
	barva : function (co) {
		...
		if (co.id)
			x.id = co.id;
		if (co.className)
			co.className = "nastaveniBarvy " + co.className;
		else
			co.className = "nastaveniBarvy";
		...
		x.addEventListener("mousedown", function (e) {
			...
			var bar = x.value;
			if (bar == "transparent")
				bar = "rgba(255,255,255,0)";
			if (!bar)
				bar = "rgba(255,255,255,0)";
			if (bar.substr(0, 4) == "rgba")
				rgb = bar.slice(5, -1).split(",");
						...
						children : [
							...
							tfw.icon({
								...
								action : function (e) {
									...
									if (re.test($($("dlgPaleta").value).childNodes[1].innerHTML))
			...
			$("dlgPaleta").add(d = tfw.div({
						...
						children : [
							tfw.span({
								...
								style : "float:left;background-image:-webkit-linear-gradient(rgba(" + barva.v + "),rgba(" + barva.v + ")),url(pics/vzorek.png)"
							}),
							tfw.span({
		...
		if (co.value)
			x.barvaAktualniAVychozi(co.value, co.value);
		return x;
	},
	/**
	 * @todo Remove dependencies on Triobo
	...
	barvaSLegendou : function (co) {
		...
		l.style.display = "inline-block";
		if (co.legenda)
			l.innerHTML = co.legenda;
		if (co.legend)
			l.innerHTML = co.legend;
		if (co.legendaSirka)
			l.style.width = co.legendaSirka;
		if (co.legendStyle)
			l.style.cssText = co.legendStyle;
		x.add(l);
